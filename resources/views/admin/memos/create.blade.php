<x-layouts.app>
    ...
    <div class="card">
        <form action="{{route('admin.memos.store')}}" method="POST" class="space-y-4">
            @csrf
            
            <flux:input readonly variant="filled" label="Nro Control:" name="nro_control" value="{{$controlNumber}}" />

            <flux:input readonly variant="filled" label="Fecha de creación:" name="date_created" value="{{date('d-m-Y')}}" />
            ...
            <flux:input label="Titulo:" name="title" value="{{old('title')}}" placeholder="Escriba el titulo del memo." />
            ...
            foreach ($departments as $department) {
                $verificar = (is_array(old('departments')) && in_array($department->id, old('departments'))) ? 'checked' : '' ;
                $contador++;
                
                if ($contador ===  $departments_por_columna ) {
                    echo '<div class="columna">';
                    echo '<div class="noticia">';
                    echo '<label class="flex items-center space-x-2"><input type="checkbox" name="departments[]" value="'.$department->id.'" class="item-checkbox" '.$verificar.'><span>'.$department->name.'</span></label>';
                    echo '</div>';
                }
            ...
            <div class="form-group" id="input-otros" style="display: none;">
                <flux:input label="Especifique:" name="addressed_to" value="{{old('addressed_to')}}" id="valor_otros" placeholder="Escriba a quien va dirigido el memo." />
            </div>
