<x-layouts.app>
    ...
    <div class="card">
        <form action="{{route('admin.projects.store')}}" method="POST" class="space-y-4">
            ...
            <flux:input label="Titulo" name="title" value="{{old('title')}}" placeholder="Escriba el titulo del proyecto." />
            ...
            <flux:input label="Fecha de Inicio:" type="date" max="2999-12-31" name='date_start' value="{{old('date_start')}}"/>
            
            <flux:input label="Fecha de Culminación:" type="date" max="2999-12-31" name='date_finish' value="{{old('date_finish')}}"/>
            ...
            <flux:select label='Estatus' name="status" placeholder="Elija por favor...">
                <flux:select.option value="Iniciado" :selected="old('status') == 'Iniciado'">Iniciado</flux:select.option>
                <flux:select.option value="En Ejecución" :selected="old('status') == 'En Ejecución'">En Ejecución</flux:select.option>
                <flux:select.option value="Pendiente" :selected="old('status') == 'Pendiente'">Pendiente</flux:select.option>
                <flux:select.option value="Culminado" :selected="old('status') == 'Culminado'">Culminado</flux:select.option>
            </flux:select>
            ...
            @php
                // Obtener el personal previamente seleccionado de errores de validación, si los hay
                $oldStaffs = old('staffs', []);
            @endphp
                ...
                @foreach ($staffs as $staff)
                    <div class="noticia">
                        <label class="flex items-center space-x-2">
                            ...
                            <span>{{ $staff->name .' - '.$staff->department->name}}</span>
                        </label>
                    </div>
                @endforeach
            </div>
